        ...
        match request {
            ClientRequest::InitializeRequest(_req) => {
                ...
                let info: rmcp::model::InitializeResult = serde_json::from_value(response_json)
                    .map_err(|e| rmcp::model::ErrorData::new(
            ...
            ClientRequest::CallToolRequest(req) => {
                ...
                let running_map: Arc<Mutex<HashMap<mcp_types::RequestId, codex_protocol::mcp_protocol::ConversationId>>> =
                    Arc::new(Mutex::new(HashMap::new()));

                let req_id_val = serde_json::to_value(&context.id).map_err(|e| rmcp::model::ErrorData::new(
                    rmcp::model::ErrorCode(-32004),
                    format!("id ser error: {e}"),
                    None,
                ))?;
                let mcp_req_id: mcp_types::RequestId = serde_json::from_value(req_id_val)
                    .map_err(|e| rmcp::model::ErrorData::new(
                ...
                match name {
                    "codex" => {
                        ...
                        let param: crate::codex_tool_config::CodexToolCallParam = serde_json::from_value(args_value)
                            .map_err(|e| rmcp::model::ErrorData::new(
                        ...
                        let (initial_prompt, cfg) = param.into_config(self._codex_linux_sandbox_exe.clone())
                            .map_err(|e| rmcp::model::ErrorData::new(
                    ...
                    "codex-reply" => {
                        let args_map = req.params.arguments.unwrap_or_default();
                        let args_value = serde_json::Value::Object(args_map);
                        let param: crate::codex_tool_config::CodexToolCallReplyParam = serde_json::from_value(args_value)
                            .map_err(|e| rmcp::model::ErrorData::new(
                                rmcp::model::ErrorCode(-32602),
                                format!("invalid params: {e}"),
                                None,
                        ...
                        let conv_uuid = uuid::Uuid::parse_str(&param.conversation_id).map_err(|e| rmcp::model::ErrorData::new(
                            rmcp::model::ErrorCode(-32602),
                            format!("invalid conversation_id: {e}"),
                            None,
                        ))?;
                        let conv_id = codex_protocol::mcp_protocol::ConversationId::from(conv_uuid);
                        let conversation = self.conversation_manager.get_conversation(conv_id).await
                            .map_err(|e| rmcp::model::ErrorData::new(
                    ...
                    match out_rx.recv().await {
                        Some(OutgoingMessage::Response(resp)) => {
                            if resp.id == mcp_req_id {
                                let val = serde_json::to_value(resp.result).map_err(|e| rmcp::model::ErrorData::new(
                                ...
                                let val2 = serde_json::to_value(call).map_err(|e| rmcp::model::ErrorData::new(
                                    rmcp::model::ErrorCode(-32002),
                                    format!("serde error: {e}"),
                                    None,
                                ))?;
                                let rmcp_call: rmcp::model::CallToolResult = serde_json::from_value(val2)
                                    .map_err(|e| rmcp::model::ErrorData::new(
                                        rmcp::model::ErrorCode(-32003),
                                        format!("schema error: {e}"),
                                        None,
                                ...
                                if req0.method == mcp_types::ElicitRequest::METHOD {
                                    let (message, requested_schema, meta_opt) = match &req0.params {
                                        Some(p) => {
                                            ...
                                            let schema_obj = p.get("requestedSchema").and_then(|v| v.as_object()).cloned().unwrap_or_default();
                                            ...
                                            let meta = if extras.is_empty() { None } else { Some(rmcp::model::Meta(extras)) };
                                            (msg, schema_obj, meta)
                                        }
                                        None => ("".to_string(), serde_json::Map::new(), None),
                                    };

                                    let request = rmcp::model::ServerRequest::CreateElicitationRequest(
                                        rmcp::model::CreateElicitationRequest {
                                            method: Default::default(),
                                            params: rmcp::model::CreateElicitationRequestParam { message, requested_schema },
                                            extensions: Default::default(),
                                        },
                                    );
                                    let options = rmcp::service::PeerRequestOptions { timeout: None, meta: meta_opt };
                                    let handle = match peer.send_request_with_option(request, options).await {
                                        Ok(h) => h,
                                        Err(e) => {
                                            let err = mcp_types::JSONRPCErrorError { code: -32603, message: format!("{e}"), data: None };
                                    ...
                                    match handle.await_response().await {
                                        Ok(rmcp::model::ClientResult::CreateElicitationResult(res)) => {
                                            let decision = match res.action {
                                                rmcp::model::ElicitationAction::Accept => codex_core::protocol::ReviewDecision::Approved,
                                                rmcp::model::ElicitationAction::Decline | rmcp::model::ElicitationAction::Cancel => codex_core::protocol::ReviewDecision::Denied,
                                            };
                                            let back = serde_json::json!({ "decision": decision });
                                            outgoing_for_cb.notify_client_response(req0.id.clone(), back).await;
                                        }
                                        Ok(_) => {
                                            let err = mcp_types::JSONRPCErrorError { code: -32603, message: "unexpected elicitation result variant".to_string(), data: None };
                                            outgoing_for_cb.send_error(req0.id.clone(), err).await;
                                        }
                                        Err(e) => {
                                            let err = mcp_types::JSONRPCErrorError { code: -32603, message: format!("{e}"), data: None };
                                    ...
                                    let err = mcp_types::JSONRPCErrorError { code: -32601, message: format!("unsupported server request method: {}", req0.method), data: None };
                                    outgoing_for_cb.send_error(req0.id.clone(), err).await;
                                }
                            });
                        }
                        Some(OutgoingMessage::Notification(note)) => {
                            ...
                            if logger == "progress/update" { logger = "progress".to_string(); }
            ...
            _ => Ok(rmcp::model::ServerResult::EmptyResult(rmcp::model::EmptyObject {})),
        }
    }
...
pub async fn run_official_server(
    ...
    let running: rmcp::service::RunningService<rmcp::service::RoleServer, _> = service.serve(rmcp::transport::stdio()).await?;
